...
class ReferenceServerSetup
  ...
  def initialize
    ...
    self.hostname = "Rogue01.local"
    self.port = 4567
    self.pass_type_identifier = "pass.com.theatrePass.A-Level-CompSciProj"
  end
  ...
  def add_user(email, name, seatRef)
    users = self.db[:users]
    now = DateTime.now
    users.insert(:email => email, :name => name, :seatRef => seatRef, :created_at => now, :updated_at => now)
    puts "<#User email: #{email} name: #{name} seatRef: #{seatRef}>"
  end
  ...
  def add_pass_for_user(user_id)
    ...
    add_pass(serial_number, authentication_token, "pass.com.theatrePass.A-Level-CompSciProj", user_id)
  end
...
optparse = OptionParser.new do |opts|
  ...
  options[:add_user] = []
  opts.on('--add-user email,name,seatRef', Array, "Add a user to the database") do |u|
    options[:add_user] = u
  end
